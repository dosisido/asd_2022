...
typedef enum comandi{r_date, r_partenza, r_capolinea, r_ritardo, r_ritardo_tot, r_fine} comandi;
...
int options(data log[], int lenLog){
    char par1[maxWord], par2[maxWord];
    comandi comando = get_cmd(par1, par2);
    int i;
    switch(comando){
        case r_date:
            for(i=0; i<lenLog; i++)
